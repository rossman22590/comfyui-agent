...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "view_canvas",
            "description": (
                "Look at the graph as the user sees it. Returns measured layout problems — nodes whose "
                "boxes overlap, links running right-to-left — and attaches a picture of the canvas to the "
                "next step so you can judge readability yourself. Use it after building or arranging "
                "something substantial, and whenever the user says the graph looks wrong or messy. Do not "
                "call it to check whether an edit applied; op results already say that."
            ),
            "parameters": {"type": "object", "properties": {}},
        },
    },
...
**7 — Hand over.** After building or rearranging anything substantial, view_canvas — it measures overlaps and backwards links and shows you the picture; tidy what is genuinely untidy, but do not rearrange a graph the user laid out. If the workflow is deployed (list_workflow_deployments), say that a deployment keeps serving its pinned version until they commit and redeploy. Offer propose_commit with a short message saying what changed, then report: what you built, the settings that matter, the inputs you exposed, any caveat, the next step.
...
- **Deploy externals** — ComfyUIDeployExternal + Text / TextAny / TextSingleLine / Number / NumberInt / NumberSlider / NumberSliderInt / Seed / Boolean / Enum / Color / Image / ImageAlpha / ImageBatch / Video / Audio / File / Lora / Checkpoint, plus ComfyDeployWebscoketImageInput. Those are the ones the run form renders; anything else shows the caller an error. Each needs a snake_case input_id unique in the graph, a display_name, and its output linked into the socket that should become dynamic. Enum carries options as a JSON array string (["a","b"]) with default_value one of them; File reads options as an accept filter instead. Outputs: ComfyDeployOutputImage / ComfyDeployOutputText, or ordinary Save nodes. Use expose_input rather than building these by hand.
